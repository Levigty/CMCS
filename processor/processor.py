...
class Processor(IO):
    ...
    def load_data(self):
        ...
        if self.arg.mem_train_feeder_args:
            ...
            self.data_loader['mem_train'] = torch.utils.data.DataLoader(
                dataset=mem_train_feeder(**self.arg.mem_train_feeder_args),
                batch_size=self.arg.batch_size,
                shuffle=True,
                pin_memory=True,    # set True when memory is abundant
                num_workers=self.arg.num_worker * torchlight.ngpu(
        ...
        if self.arg.mem_test_feeder_args:
            ...
            self.data_loader['mem_test'] = torch.utils.data.DataLoader(
                dataset=mem_test_feeder(**self.arg.mem_test_feeder_args),
                batch_size=self.arg.test_batch_size,
                shuffle=False,
                pin_memory=True,
                num_workers=self.arg.num_worker * torchlight.ngpu(
    ...
    def get_parser(add_help=False):
        ...
        parser.add_argument('--mem_train_feeder', default='feeder.feeder', help='memory train data loader will be used for knn')
        parser.add_argument('--mem_test_feeder', default='feeder.feeder', help='memory test data loader will be used for knn')
        ...
        parser.add_argument('--mem_train_feeder_args', action=DictAction, default=dict(), help='the arguments of data loader for training for knn')
        parser.add_argument('--mem_test_feeder_args', action=DictAction, default=dict(), help='the arguments of data loader for test for knn')
